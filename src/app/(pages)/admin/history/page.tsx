...
export default function HistoricoPedidos() {
  ...
  useEffect(() => {
    const getPedidos = async () => {
      ...
      setPedidos(
        data.docs.map((doc) => ({ ...doc.data(), id: doc.id })) as pedido[]
      );
    };
    getPedidos();
  }, []);
  ...
  const downloadCSV = () => {
    const csvContent =
      ...
      "data,comprador,email,nome,quantidade,preco-unidade\n" +
      Pedidos.slice()
        .sort((a, b) => {
        ...
        .map(
          (pedido) =>
            `"${pedido.dataHora}","${pedido.comprador}","${pedido.email}","${pedido.nome}","${pedido.quantidade}","${formatToBRL(pedido.preco)}"`
          )
        .join("\n");
    ...
    <div className="bg-[#FFF4EF] min-h-screen w-full flex flex-col">
      ...
      <main className="flex flex-col items-center justify-start ">
        <div className="w-full">
          ...
          <div className="relative flex items-center w-4/5 sm:w-full max-w-xl mx-auto bg-white border border-conpec-blue text-conpec-blue rounded-2xl shadow-md focus-within:text-conpec-primary focus-within:ring-2 focus-within:border-conpec-primary focus-within:ring-[#FF3D00] transition-all">
            <Search
              className="absolute left-4 top-1/2 transform -translate-y-1/2"
              size={22}
            />
            <input
              className="w-full py-3 pl-12 pr-4 text-[16px] rounded-2xl bg-transparent placeholder-conpec-blue text-conpec-primary focus:placeholder-conpec-primary focus:outline-none focus:ring-0"
                    ...
                    <div key={index}>
                      ...
                      <div className="pr-6 md:px-10 space-y-2">
                        {items.map((item, i) => (
                          <div
                            key={i}
                            className="flex flex-row justify-between font-Poppins font-bold text-xl md:text-2xl space-x-1"
                          >
                            <span className="text-[#FF9633] w-2/6">
                              {item.cliente}
                            </span>
                            <span className="text-[#FF9633] text-center w-1/2">
                              {item.produto}
                            </span>
                            <span className="text-[#FF9633] w-1/6">
        ...
        <div className="fixed bottom-8 right-8 z-50">
          <button
            className="bg-conpec-primary rounded-full flex flex-col items-center justify-center w-20 h-20 shadow-lg"
            onClick={downloadCSV}
          >
